...
(function () {
  ...
  function init() {
    ...
    // load, so preloading put a multi-megabyte download on the wire while the
    // guest was still looking at the unopened envelope, competing with every
    // photograph for a phone's bandwidth. Nothing needs it until the envelope
    // is tapped, and start() already loads and waits for canplay by then.
    audio.preload = "none";
    audio.volume = 0;
    ...
    // With preload "none" nothing is fetched until start(), so this now fires
    // after the button exists and has to take it back down again.
